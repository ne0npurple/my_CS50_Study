...
void lock_pairs(void)
{
    ...
    for (int i = 0; i < pair_count; i++)
    {
        ...
        wn = find(pairs[i].winner, winner_arr);
        lsr = find(pairs[i].loser, winner_arr);
        if (winner_arr[lsr] == wn)
        {
            continue;
        }
        winner_arr[lsr] = wn;
